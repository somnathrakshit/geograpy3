...
class TestLocationHierarchy(Geograpy3Test):
    ...
    def testDistance(self):
        ...
        test_points_rad = np.array(
            [[radians(x[0]), radians(x[1])] for x in test_points]
        )

        tree = BallTree(test_points_rad, metric="haversine")
        ind, results = tree.query_radius(
            test_points_rad, r=test_radius / earth_radius, return_distance=True
        )
        if self.debug:
            print(ind)
    ...
    def checkLocationListWithDistances(
        self,
    ...
    def testClosestLocation(self):
        ...
        self.checkLocationListWithDistances(
            countryListWithDistances, 2, "Luxembourg", 244
        )
