...
import geograpy
from geograpy.locator import City, CountryManager, Location, LocationContext, Locator
...
class TestLocator(Geograpy3Test):
    ...
    def testCityLookup(self):
        ...
        """
        cityLookupRecords = self.lookupQuery(
            "CityLookup", "label in ('Berlin','Paris','Athens','Singapore')"
        )
        expected = [
            ("Berlin", 3644000),
            ("Paris", 2175000),
...
[[https://github.com/somnathrakshit/geograpy3 © 2020-2021 geograpy3 project]]"""
        plantUml = uml.tableListToPlantUml(
            tableList, title=title, packageName="geograpy3"
        )
        showUml = True
...
February 3-5, 2020"""
    ...
    def testDelimiters(self):
        ...
        ddls = [
            "DROP VIEW IF EXISTS allNames",
            """CREATE VIEW allNames as select name from countries
    ...
    def testIssue_59_db_download(self):
        ...
        def downloadAndTestDB(
            config: StorageConfig, loc: Locator = None, forceUpdate: bool = False
        ):
